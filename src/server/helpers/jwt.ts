...
const
    ...
    signToken = (payload:JWTPayload):Promise<string> => new SignJWT(payload)
        ...
        .setProtectedHeader({alg: APP_KEYPAIR_ALG as string})
    ...
    verifyToken = (token:string):Promise<JWTVerifyResult<JWTPayload>> => jwtVerify(token, pubk, {
        // validate algorithm
        algorithms: [ APP_KEYPAIR_ALG as string ],
